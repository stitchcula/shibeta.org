...
class Usr {
    constructor(handle){//session缓存10天，usrMsg缓存1天，mongo永久
        ...
        if(handle.session.uin){
            this.status=(handle.session.ol>new Date().getTime())?2:1//2为已登录、1为需要验证密码
            var _=yield this.handle.redis.hgetall(handle.session.uin)
            if(!_){
                this.usrMsg=yield this.usrDb.findOne({uin:handle.session.uin})
                this.handle.redis.hmset(handle.session.uin,this.usrMsg)
            }else this.usrMsg=_
        }else{//未登陆游客
            this.usrMsg=this.baseUsrMsg
            this.status=0
        }
    }
    enBase64(str){
